...
def add():
    ...
    nameFruit = pypi.inputStr(prompt='Masukkan nama buah: ', applyFunc=lambda x: x.capitalize(), blockRegexes = '@1234567890' )
...
def delete():
    ...
    index = pypi.inputInt(prompt='Masukkan index buah yang ingin dihapus: ',lessThan=len(listFruit)-1)
...
def buy():
    ...
    while True:
        ...
        index = pypi.inputInt(prompt='Masukkan index buah yang ingin dibeli: ',lessThan=len(listFruit)-1)
        nameFruit = listFruit[f'buah-{index}']
        countFruit = pypi.inputInt(prompt='Masukkan jumlah buah yang ingin dibeli: ', lessThan=nameFruit[2])
        ...
        # Selain itu, tambahkan ke 'listChart'
            ...
            reorder = pypi.inputYesNo(prompt='Apakah anda ingin membeli buah yang lain? (yes/no): ')
            if reorder.lower() == "no":
                break
    ...
    while True:
        ...
        pay = pypi.inputInt(prompt='Masukkan jumlah uang anda: ')
...
#         )
        ...
        menuNumber = pypi.inputMenu(prompt='----Selamat Datang di Pasar Buah----:\nSilahkan pilih menu yang akan dijalankan\n', choices=choice, numbered=True)
        # Fitur menampilkan daftar buah
        if menuNumber == 'Menampilkan daftar buah':
            show(listFruit, printFormat)
        # Fitur menambahkan buah
        elif menuNumber == 'Menambah buah':
            add()
        # Fitur menghapus buah
        elif menuNumber == 'Menghapus buah':
            delete()
        # Fitur membeli buah
        elif menuNumber == 'Membeli buah':
            buy()
        # Fitur exit program
        else:
            sys.exit()
